...
module.exports.run = async function({ api, args, event, permssion }) {
  ...
  if(args[0] == "find" || args[0] == "tìm"){
    const lengthchar = (await axios.get(`https://6821df2e-d4cb-44c6-abb3-185e21d7d2fb.id.repl.co/anime_characters?id=${args[1]}&tid=${event.threadID}&uid=${event.senderID}&api_key=${global.config.api_key.key_game}`)).data
    if (lengthchar.status == false) return api.sendMessage(lengthchar.msg, event.threadID, event.messageID)
    const t = (await axios.get(lengthchar.anime.image_Anime, {
        responseType: "stream"
      })).data;
    var msg = ({
      body: `ID NHÂN VẬT ${args[1]}, MÀU MẶC ĐỊNH ${lengthchar.anime.color_bg}`,
      attachment: t
    })
    return api.sendMessage(msg, event.threadID, event.messageID)
  }
  else if(args[0] == "list"){
    const alime = (await axios.get('https://6821df2e-d4cb-44c6-abb3-185e21d7d2fb.id.repl.co/anime_characters_list')).data
  ...
  const fs =  require('fs-extra')
   if (!fs.existsSync(__dirname +
        `/cache/UTM-Avo.ttf`)) {
        let getfont = (await axios.get(`https://raw.githubusercontent.com/duongcongnam/font/main/UTM-Avo.ttf`, { responseType: "arraybuffer" })).data;
        fs.writeFileSync(__dirname + `/cache/UTM-Avo.ttf`, Buffer.from(getfont, "utf-8"));
      }
         if (!fs.existsSync(__dirname +
      `/cache/phenomicon.ttf`)) {
      let getfont2 = (await axios.get(`https://raw.githubusercontent.com/duongcongnam/font/main/phenomicon.ttf`, { responseType: "arraybuffer" })).data;
      fs.writeFileSync(__dirname + `/cache/phenomicon.ttf`, Buffer.from(getfont2, "utf-8"));
    };
  if (!fs.existsSync(__dirname +
      `/cache/CaviarDreams.ttf`)) {
      let getfont3 = (await axios.get(`https://raw.githubusercontent.com/duongcongnam/font/main/CaviarDreams.ttf`, { responseType: "arraybuffer" })).data;
...
module.exports.handleReply = async function({ api, event, args, handleReply, client, __GLOBAL, Threads, Users, Currencies }) {
  ...
  let pathAva = __dirname + `/cache/avatar_2.png`;
  ...
  const msg = ({
    ...
    if(id_ == "random" || id_ == "Random"){
      var id = Math.floor(Math.random() * 903)
    } else {
      var id = id_
    }
    if (color_ == "no" || color_ == "No") {
        const lengthchar = (await axios.get(`https://6821df2e-d4cb-44c6-abb3-185e21d7d2fb.id.repl.co/anime_characters?id=${id-1}&tid=${event.threadID}&uid=${event.senderID}&api_key=${global.config.api_key.key_game}`)).data
      if (lengthchar.status == false) return api.sendMessage(lengthchar.msg, event.threadID, event.messageID)
      var color = lengthchar.anime.color_bg;
    } else {
      var color = color_;
    }
    const lengthcharr = (await axios.get(`https://6821df2e-d4cb-44c6-abb3-185e21d7d2fb.id.repl.co/anime_characters?id=${id-1}&tid=${event.threadID}&uid=${event.senderID}&api_key=${global.config.api_key.key_game}`)).data
     let background = (await axios.get(encodeURI(`${loz[khung - 1]}`), { responseType: "arraybuffer" })).data;
    fs.writeFileSync(pathImg, Buffer.from(background, "utf-8"));
    let ava = (await axios.get(encodeURI(`${lengthcharr.anime.image_Anime}`), { responseType: "arraybuffer" })).data;
