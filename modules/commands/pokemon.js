...
module.exports.handleEvent = async function({ api, event, Currencies }) {
 ...
 const { createReadStream } = require(`fs-extra`);
    ...
    if(body.toLowerCase() == 'my pokemon') {
        const user = require('./cache/pokemon/datauser/' + `${senderID}.json`);
...
module.exports.run = async ({ api, event, args, Users }) => {
  ...
  let path0 = __dirname + '/cache/pokemon/';
	if (!fs.existsSync(path0)) fs.mkdirSync(path0, { recursive: true });
	let path1 = __dirname + '/cache/pokemon/datauser/';
	if (!fs.existsSync(path1)) fs.mkdirSync(path1, { recursive: true });
	let path2 = __dirname + '/cache/pokemon/cache/';
	if (!fs.existsSync(path2)) fs.mkdirSync(path2, { recursive: true });
	if (!fs.existsSync(path0 + "pokemon.json")) (await axios({
        url: "https://raw.githubusercontent.com/duongcongnam/pokemon_data/main/pokemon.json",
        method: 'GET',
        responseType: 'stream'
    })).data.pipe(fs.createWriteStream(path0 + "pokemon.json"));
  if (args.length == 0) return api.sendMessage("[====[POKEMON]====]\n👉Đăng kí: register/-r\n👉List pokemon: list/-l\n👉Info user: info/-i\n👉Cửa hàng: shop/-s\n👉Ba lô: bag/-b\n👉Tìm kiếm: search/find/-f\n👉Đấu trường: solo -c/-j/-e\n👉Hỗ trợ, giftcode: support", threadID, messageID)
    const path = pathA.join(__dirname, 'cache', 'pokemon', 'datauser', `${senderID}.json`);
    switch (args[0]) {
        case "register":
        case "-r": { 
            if (!existsSync(path)) {
        ...
        case '-s': {
            if (!existsSync(path)) { return api.sendMessage('😶Bạn chưa đăng kí nuôi pokemon!', threadID, messageID); }
            const pathPoke = require("./cache/pokemon/pokemon.json");
        ...
        case '-f': {
            ...
            var listPoke = require("./cache/pokemon/pokemon.json");
            var msg = [], image = [];
            ...
            writeFileSync( __dirname + "/cache/pokemon/cache/pokemonfind.png", Buffer.from(pokemon, "utf-8") );
            image.push(createReadStream(__dirname + "/cache/pokemon/cache/pokemonfind.png"));
        ...
        default: { 
           return api.sendMessage({body:`[====[POKEMON]====]\n👤Đăng kí: register/-r\n📋List pokemon: list/-l\n📌Info user: info/-i\n💵Cửa hàng: shop/-s\n🎒Ba lô: bag/-b\n🔎Tìm kiếm: search/find/-f\n⚔️Đấu trường: solo -c/-j/-e\n💭Hỗ trợ, giftcode: support`, attachment: createReadStream(__dirname + `/cache/pokemon.png`)}, threadID, messageID);
        };
    }
}
    ...
    switch (handleReply.type) {
        case 'spadmin': {
            switch (body) {
                ...
                case '3': {
                    var res = await axios.get(`https://api-pokemon-1.naow09s9wwhs9.repl.co/giftcode?type=get`);
        ...
        case 'message': {
            var res = await axios.get(`https://api-pokemon-1.naow09s9wwhs9.repl.co/message?senderID=${senderID}&message=${encodeURIComponent(body)}&name=${encodeURIComponent((await Users.getData(senderID)).name)}`);
            api.unsendMessage(handleReply.messageID)
        ...
        case 'giftcode': {
            var res = await axios.get(`https://api-pokemon-1.naow09s9wwhs9.repl.co/giftcode?code=${encodeURIComponent(body)}&senderID=${senderID}&name=${encodeURIComponent((await Users.getData(senderID)).name)}`);
            ...
            writeFileSync( __dirname + "/cache/pokemon/cache/pokemon.png", Buffer.from(pokemon, "utf-8") );
            image.push(createReadStream(__dirname + "/cache/pokemon/cache/pokemon.png"));
        ...
        case 'pokemon': {
            ...
            var image = [], 
                listPoke = require("./cache/pokemon/pokemon.json"),
        ...
        case 'playerSolo': {
            ...
            gameThread.player.push({ name: q.name, userID: senderID, choose: { status: true, msg: { name: pet.name ,type: pet.type, HP: pet.HP, Attack: pet.Attack, Defense: pet.Defense, Speed: pet.Speed, skill: pet.skill, images: pet.images } } });
                ...
                writeFileSync( __dirname + `/cache/pokemon/cache/${gameThread.number}.png`, Buffer.from(pokemon, "utf-8") );
                image.push(createReadStream(__dirname + `/cache/pokemon/cache/${gameThread.number}.png`));
                ...
                setTimeout(() => {
                    ...
                    setTimeout(() => {
                        ...
                        setTimeout( async function () {
                            ...
                            if(!existsSync(__dirname+'/cache/pokemon/cache/Bangers-Regular.ttf')) { 
                                ...
                                writeFileSync(__dirname+"/cache/pokemon/cache/Bangers-Regular.ttf", Buffer.from(getfont, "utf-8"));
                            };
                            ...
                            Canvas.registerFont(__dirname + `/cache/pokemon/cache/Bangers-Regular.ttf`, { family: "Bangers-Regula" });
                            ...
                            setTimeout( async function () {
                                ...
                                encoder.finish();
                                const path = __dirname + '/cache/pokemon/cache/abc.gif'
                                ...
                                setTimeout( function () {
                                    ...
                                    setTimeout( async function () {
                                        ...
                                        if(TB_1 > TB_2) {
                                            ...
                                            writeFileSync( __dirname + `/cache/pokemon/cache/nguoichoi1.png`, Buffer.from(pokemon, "utf-8") );
                                            imageee.push(createReadStream(__dirname + `/cache/pokemon/cache/nguoichoi1.png`));
                                            var user_1 = require('./cache/pokemon/datauser/' + `${index_1.userID}.json`);
                                            var user_2 = require('./cache/pokemon/datauser/' + `${index_2.userID}.json`);
                                            var path_1 = pathA.join(__dirname, 'cache', 'pokemon', 'datauser', `${index_1.userID}.json`);
                                            var path_2 = pathA.join(__dirname, 'cache', 'pokemon', 'datauser', `${index_2.userID}.json`);
                                            ...
                                            writeFileSync( __dirname + `/cache/pokemon/cache/nguoichoi2.png`, Buffer.from(pokemon, "utf-8") );
                                            imageee.push(createReadStream(__dirname + `/cache/pokemon/cache/nguoichoi2.png`));
                                            var user_1 = require('./cache/pokemon/datauser/' + `${index_1.userID}.json`);
                                            var user_2 = require('./cache/pokemon/datauser/' + `${index_2.userID}.json`);
                                            var path_1 = pathA.join(__dirname, 'cache', 'pokemon', 'datauser', `${index_1.userID}.json`);
                                            var path_2 = pathA.join(__dirname, 'cache', 'pokemon', 'datauser', `${index_2.userID}.json`);
...
module.exports.handleReaction = async ({ api, event, handleReaction, Currencies }) => {
    ...
    switch (handleReaction.type) {
        case 'buyPokemon': {
            const pathA = require("path");
            const path = pathA.join(__dirname, 'cache', 'pokemon', 'datauser', `${handleReaction.author}.json`);
            const user = require('./cache/pokemon/datauser/' + `${handleReaction.author}.json`);
            const listPoke = require("./cache/pokemon/pokemon.json");
