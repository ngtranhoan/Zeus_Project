    var request = require("request");const { readdirSync, readFileSync, writeFileSync, existsSync, copySync, createWriteStream, createReadStream, mkdirSync } = require("fs-extra");
  ...
  let path = __dirname + '/cache/baucua/';
	if (!existsSync(path)) mkdirSync(path, { recursive: true });
        if (!existsSync(__dirname + '/cache/baucua/ga.jpg')) {
            request('https://i.imgur.com/z9S53tI.jpg').pipe(createWriteStream(__dirname + '/cache/baucua/ga.jpg'));
        }
        if (!existsSync(__dirname + '/cache/baucua/tom.jpg')) {
            request('https://i.imgur.com/sbnCjxg.jpg').pipe(createWriteStream(__dirname + '/cache/baucua/tom.jpg'));
        }
        if (!existsSync(__dirname + '/cache/baucua/bau.jpg')) {
            request('https://i.imgur.com/UitT2gj.jpg').pipe(createWriteStream(__dirname + '/cache/baucua/bau.jpg'));
        }
        if (!existsSync(__dirname + '/cache/baucua/cua.jpg')) {
            request('https://i.imgur.com/MaycUIw.jpg').pipe(createWriteStream(__dirname + '/cache/baucua/cua.jpg'));
        }
        if (!existsSync(__dirname + '/cache/baucua/ca.jpg')) {
            request('https://i.imgur.com/lSjlQA6.jpg').pipe(createWriteStream(__dirname + '/cache/baucua/ca.jpg'));
        }
        if (!existsSync(__dirname + '/cache/baucua/nai.jpg')) {
            request('https://i.imgur.com/dRGnH4U.jpg').pipe(createWriteStream(__dirname + '/cache/baucua/nai.jpg'));
        }
        if (!existsSync(__dirname + '/cache/baucua/baucua.gif')) {
            request('https://i.imgur.com/BgaPweh.gif').pipe(createWriteStream(__dirname + '/cache/baucua/baucua.gif'));
        }
    };
...
var full = [];
    module.exports.run = async function({ api, event, args, Currencies }) { var out = (msg) => api.sendMessage(msg,event.threadID, event.messageID);
        const slotItems = ["ga", "tom", "bau", "cua", "ca", "nai"];
            ...
            if (moneyBet < 50) return api.sendMessage("[𝑷𝑮🐧] => Số tiền đặt không được dưới 50 đô!", event.threadID, event.messageID);
    ...
    for (let i = 0; i < 3; i++) number[i] = slotItems[Math.floor(Math.random() * slotItems.length)];
        var itemm;
            ...
            api.sendMessage({body:"[𝑷𝑮🐧] => Đang Đập, À Không Đang Lắc!",attachment: createReadStream(__dirname + "/cache/baucua/baucua.gif")},event.threadID,async (error,info) => {
